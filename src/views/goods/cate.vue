<template>
  <div class="cate-container">
    <div class="top-btn"><el-button type="primary" @click="showAddClassifyDialog">新增分类</el-button></div>

    <!-- 添加分类的对话框 -->
    <div class="addClassifyDialog">
      <el-dialog title="添加分类" :visible.sync="addClassifyVisiable" width="50%" @close="addDialogClose">
        <!-- 添加分类的表单 -->
        <el-form :model="addClassifyForm" :rules="addClassifyFormRules" ref="addClassifyDialog" label-width="100px">
          <el-form-item label="分类名称" prop="classifyName"><el-input v-model="addClassifyForm.classifyName"></el-input></el-form-item>
    ...
    <div class="tree-table">
      ...
      >
        ...
        <template slot="operation" slot-scope="scope">
          <el-button type="primary" icon="el-icon-edit" size="mini" @click="editClassify(scope.row)">编辑</el-button>
          <el-button type="danger" icon="el-icon-delete" size="mini" @click="deleteClassify(scope.row)">删除</el-button>
        </template>
      </tree-table>
    </div>
    <!-- 分页组件 -->
    <div class="pagination"><el-pagination background layout="prev, pager, next" :total="total" :page-size="4" @current-change="handlePageChange"></el-pagination></div>
  </div>
</template>
<script>
import '@/mock/category';
import { categoryList, parentCategoryList, addCategory, updateCategory, categoryDetail, deleteCategory } from '@/api/category';
  ...
  methods: {
    ...
    submitForm: async function() {
      ...
      this.$refs.addClassifyDialog.validate(async valid => {
        ...
        if (this.mode == 'add') {
          const { code, message } = await addCategory(this.addClassifyForm);
          if (code != 200) {
          ...
          const { code, message } = await updateCategory(this.addClassifyForm);
          if (code != 200) {
    ...
    deleteClassify: async function(row) {
      this.$confirm('是否要删除当前分类', '删除提醒提示', {
        ...
        .then(async () => {
          const { code, message } = await deleteCategory({ username: row.name });
          if (code === 200) {
            this.$message.success('删除分类【' + row.name + '】成功');
            this.categoryList = [];
            this.getCateList();
          } else {
